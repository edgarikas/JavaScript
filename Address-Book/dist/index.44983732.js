...
function render() {
    ...
    if (document.querySelector(`#${contactsId}`)) document.querySelector(`#${contactsId}`).remove();
    contactsList.id = contactsId;
    contacts.filter((singleContact)=>{
        return singleContact.name.toLowerCase().includes(searchValue.toLowerCase()) || singleContact.phone.includes(searchValue);
    }).forEach((singleContact, index)=>{
        const contactEl = document.createElement("li");
        const buttons = document.createElement("div");
        const nameEl = document.createElement(singleContact.isInEdit ? "input" : "p");
        const phoneEl = document.createElement(singleContact.isInEdit ? "input" : "a");
        const deleteBtn = document.createElement("button");
        const favoriteBtn = document.createElement("button");
        const editBtn = document.createElement("button");
        if (singleContact.isInEdit) contactEl.className = singleContact.isInEdit ? "editor" : "";
        else if (singleContact.isFavorite) contactEl.className = singleContact.isFavorite ? "favorite" : "";
        phoneEl.href = `tel:${singleContact.phone}`;
        deleteBtn.textContent = "X";
        deleteBtn.classList = "delete";
        favoriteBtn.textContent = singleContact.isFavorite ? "-" : "+";
        favoriteBtn.classList = singleContact.isFavorite ? "buttons--fav" : "buttons--no-fav";
