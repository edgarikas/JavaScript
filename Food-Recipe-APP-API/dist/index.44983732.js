...
function deleteMeal() {
    if (document.querySelector("#meal")) document.querySelector("#meal").remove();
    if (document.querySelector(".notFound")) document.querySelector(".notFound").remove();
}
function setRecipesAmount(amount) {
    document.querySelector("#h2").textContent = `Your Search Result: ${amount} Recipes`;
}
function getMealList() {
    let searchInput = document.querySelector("#search-input").value;
    fetch(`https://themealdb.com/api/json/v1/1/filter.php?i=${searchInput}`).then((response)=>response.json()).then((data)=>{
        ...
        document.querySelector("#h2").textContent = `Your Search Result: 0 Recipes`;
...
function renderMealInstruction(meal) {
    ...
    link.append(a);
    mealDetailsContent.append(heading, category, recipeInstruction, recipeImg, link);
    ...
    document.querySelector(".recipe-close-btn").addEventListener("click", (e)=>{
        document.querySelector(".meal-details-content").parentElement.classList.remove(`showRecipe`);
    });
}
function getMealRecipe(e) {
    ...
    if (e.target.className === "recipe-btn") {
        ...
        // console.log(e.target.id);
        fetch(`https://themealdb.com/api/json/v1/1/lookup.php?i=${e.target.id}`).then((response)=>response.json()).then((data)=>mealRecipeInstruction(data.meals));
    }
}
